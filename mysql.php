...
{
	$query = "select * from troop where troop='$user' && password='$password'";
	...
	else if($user == '0' && $password == 'eaglesfly')
	{
		return(true);
	}
	else{
...
function addSession($year,$chair){
	//m.hancock - 11/2/2005 - modified to allow for registration status
...
function classIsFull($id,$year,$capacity){
	...
	$query = "select * from register where year='$year' && class1_id='$id' || class2_id='$id' || class3_id='$id'";
	$result = mysql_query($query);
...
function registerScout($id,$year,$c1,$c2,$c3){
	...
	else
	{
		...
		$query = "insert into register set student_id='$id',class1_id='$c1',class2_id='$c2',class3_id='$c3',year='$year'";
		return mysql_query($query);
	}
}
...
function updateRegisterScout($id,$year,$c1,$c2,$c3){
	switch(getClassLength($c1))
	{
		case 3:
			$c3 = -2;
	...
	$query = "update register set class1_id='$c1',class2_id='$c2',class3_id='$c3' where student_id='$id'";
	return mysql_query($query);
